...
export function requireAuth(): Auth {
  if (!auth) {
    throw new Error("Firebase Auth is not configured. Add your NEXT_PUBLIC_FIREBASE_* environment variables.");
  }
  return auth;
}
...
export function requireStorage(): FirebaseStorage {
  if (!storage) {
    throw new Error("Firebase Storage is not configured. Add your NEXT_PUBLIC_FIREBASE_* environment variables.");
  }
  return storage;
}
