    ...
    <>
      ...
      <main className="mx-auto max-w-md px-4 py-10 sm:px-6">
        <div className="rounded-[32px] border border-[var(--border)] bg-[rgba(255,253,252,.92)] p-8 shadow-[var(--shadow)]">
          <h1 className="text-3xl font-semibold text-[var(--primary)]">Owner login</h1>
          <p className="mt-2 text-[var(--muted)]">Log in to manage your claimed Greenwood business listing.</p>
          ...
          <form onSubmit={onSubmit} className="mt-6 grid gap-4">
            ...
            <button className="rounded-2xl bg-[var(--primary)] px-5 py-4 font-semibold text-white" disabled={loading}>{loading ? "Logging in…" : "Log in"}</button>
          </form>
          <p className="mt-5 text-sm text-[var(--muted)]">Need an account? <a className="font-semibold text-[var(--primary)]" href="/owner/signup">Create one</a></p>
        </div>
      </main>
      <Footer />
    </>
