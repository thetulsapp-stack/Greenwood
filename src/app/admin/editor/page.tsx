    ...
    <div style={cssThemeVars(settings.theme)}>
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <Link href="/admin" className="rounded-full border border-[var(--border)] bg-white px-4 py-2 text-sm font-medium text-[var(--text)]">Back to admin</Link>
              ...
              {activeTab === "site" ? (
                <div className="space-y-4">
                  ...
                  <div className="rounded-2xl border border-[var(--border)] bg-white p-4">
                    <p className="mb-3 text-sm font-semibold text-[var(--text)]">Homepage search bar</p>
                    <div className="space-y-3">
                      <Toggle checked={settings.homeSearch.visible} onChange={(visible) => setSettings({ ...settings, homeSearch: { ...settings.homeSearch, visible } })} label="Show homepage search" />
                      <TextField label="Search title" value={settings.homeSearch.title} onChange={(value) => setSettings({ ...settings, homeSearch: { ...settings.homeSearch, title: value } })} />
                      <TextAreaField label="Search description" value={settings.homeSearch.description} onChange={(value) => setSettings({ ...settings, homeSearch: { ...settings.homeSearch, description: value } })} rows={3} />
                      <TextField label="Search placeholder" value={settings.homeSearch.placeholder} onChange={(value) => setSettings({ ...settings, homeSearch: { ...settings.homeSearch, placeholder: value } })} />
                      <TextField label="Button label" value={settings.homeSearch.buttonLabel} onChange={(value) => setSettings({ ...settings, homeSearch: { ...settings.homeSearch, buttonLabel: value } })} />
                    </div>
                  </div>
                </div>
              ) : null}
