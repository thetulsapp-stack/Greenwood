            ...
            {!loading ? (
              user ? (
                <>
                  ...
                  <button onClick={() => signOut(auth)} className="hidden rounded-full border border-[var(--border)] bg-white px-4 py-2 text-sm font-medium text-[var(--muted)] sm:inline-flex">Log out</button>
                </>
              ) : (
        ...
        {open ? (
          <div className="border-t border-[var(--border)] bg-white px-4 py-4 md:hidden">
            <div className="grid gap-2">
              ...
              {!loading && user ? <button onClick={() => signOut(auth)} className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-4 py-3 text-sm font-medium text-[var(--text)]">Log out</button> : null}
            </div>
          </div>
        ) : null}
      </header>
